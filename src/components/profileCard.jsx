...
const ProfileCard = ({ imageUrl, name, githubUrl, websiteUrl, icons }) => {
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden">
      <div className="flex justify-center p-4">
        <div className="w-full h-48 relative">
          <Image className="rounded-lg transform transition-transform duration-500 ease-in-out hover:scale-110" src={imageUrl} alt={name} layout="fill" objectFit="contain" />
        </div>
      </div>
      <div className="p-4">
        <h2 className="text-center text-xl font-bold text-blue-500">{name}</h2>
        <div className="flex justify-center mt-4 space-x-4">
          ...
          <Link href={websiteUrl} passHref target="_blank" rel="noopener noreferrer">
              <button className="px-4 py-2 bg-orange-500 text-white rounded hover:bg-blue-700">Website</button>
          </Link>
        </div>
        <div className="flex justify-center mt-4 space-x-2 transform transition-transform duration-500 ease-in-out hover:scale-110">
